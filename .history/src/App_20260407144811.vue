...
// 处理新题目单选框变更
const handleNewQuestionRadioChange = (index) => {
            ...
            <template v-else>
              <a-radio-group>
                <a-radio 
                  ...
                  style="margin-right: 20px; margin-bottom: 8px;"
                  @change="handleRadioChange(question, index)"
                >
                  {{ option.text }}
                  <span class="correct-badge" v-if="option.isCorrect">✅ 正确答案</span>
                  <span class="correct-badge" v-else>❌</span>
        ...
        <div style="margin-bottom: 20px;">
          ...
          <div v-for="(option, index) in newQuestion.options" :key="index" style="margin-bottom: 12px; display: flex; align-items: center; gap: 12px;">
            <template v-if="newQuestion.type === 'multiple'">
              <a-checkbox v-model:checked="option.isCorrect"></a-checkbox>
            </template>
            <template v-else>
              <a-radio :checked="option.isCorrect" @change="handleNewQuestionRadioChange(index)"></a-radio>
            </template>
            <input 
              v-model="option.text" 
              placeholder="选项内容" 
