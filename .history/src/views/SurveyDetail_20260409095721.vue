...
const surveyDescription = ref(''); // 问卷描述

// 基本信息题目
const basicQuestions = reactive([]);

// 问卷题目
const questionQuestions = reactive([]);

// 监听题目正确答案变化，同步更新isCorrect字段
function setupQuestionListeners(questionsArray) {
  for (const question of questionsArray) {
    if (question.type === 'multiple') {
      ...
      // 监听correctAnswer变化
      Object.defineProperty(question, 'correctAnswer', {
        get() {
      ...
      // 初始化_correctAnswer
      question._correctAnswer = question.correctAnswer || '';
    } else {
      // 监听correctOption变化
      ...
      // 初始化_correctOption
      question._correctOption = question.correctOption || 0;
    }
  }
}

// 为两个题目数组分别设置监听器
setupQuestionListeners(basicQuestions);
setupQuestionListeners(questionQuestions);
...
const openPreview = () => {
  // 重置预览步骤
  previewStep.value = basicQuestions.length > 0 ? 'basic' : 'questions';
  // 打开预览模态框
...
const initSurveyData = async () => {
  ...
  if (surveyId) {
    try {
      ...
      // 回显发布设置
      settings.startTime = surveyData.startTime ? dayjs(surveyData.startTime) : null;
      ...
      if (surveyData.questions && surveyData.questions.length > 0) {
        surveyData.questions.forEach((q, index) => {
          ...
          if (q.questionType === 'QUESTION') {
            ...
            questionQuestions.push(question);
          } else {
...
watch(() => newQuestion.type, (newType) => {
  if (newType === 'multiple') {
    ...
    delete newQuestion._correctOption;
    delete newQuestion.correctAnswer;
    delete newQuestion._correctAnswer;
  } else if (newType === 'input') {
    newQuestion.correctAnswer = '';
    delete newQuestion.correctOption;
    delete newQuestion._correctOption;
    delete newQuestion.correctOptions;
    delete newQuestion._correctOptions;
  } else {
...
Object.defineProperty(newQuestion, 'correctAnswer', {
  get() {
    return this._correctAnswer || '';
  },
  set(value) {
...
newQuestion._correctAnswer = newQuestion.correctAnswer;

// 添加题目
const addQuestion = () => {
  // 重置新题目数据
  newQuestion.type = topTab.value === 'basic' ? 'input' : 'single';
  newQuestion.text = '';
...
const saveDraft = async () => {
  try {
    ...
    // 校验题目数量 - 问卷题目不能为空
    if (questionQuestions.length === 0) {
      message.error('请至少添加一个问卷题目');
      return;
    }
    ...
    for (const question of basicQuestions) {
      ...
      if (question.type !== 'input' && (!question.options || question.options.length < 2)) {
    ...
    // 校验问卷题目
    for (const question of questionQuestions) {
      if (!question.text.trim()) {
        message.error('请输入题目内容');
        return;
      }
    ...
    if (!settings.startTime) {
      message.error('答题开始时间不能为空');
      return;
    }
    if (!settings.endTime) {
      message.error('答题截止时间不能为空');
      return;
    }
    if (settings.startTime.isAfter(settings.endTime)) {
    ...
    const surveyData = {
      ...
      description: surveyDescription.value, // 问卷描述
    ...
    if (surveyId) {
      ...
      await surveyApi.update(surveyData);
    } else {
      // 如果没有问卷ID，创建新问卷
      const survey = await surveyApi.create(surveyData);
      // 保存问卷ID到路由参数
      route.params.id = survey.id;
    }
    message.success('✅ 草稿已保存，当前所有题目及设置已存储');
  } catch (error) {
...
const publishSurvey = async () => {
  try {
    ...
    // 校验时间设置
    if (settings.startTime && settings.endTime && settings.startTime.isAfter(settings.endTime)) {
      message.error('开始时间不能晚于结束时间');
      return;
    }
    ...
    const surveyData = {
      title: surveyTitle.value,
      description: surveyDescription.value, // 问卷描述
      startTime: settings.startTime ? settings.startTime.format('YYYY-MM-DD HH:mm:ss') : null,
      endTime: settings.endTime ? settings.endTime.format('YYYY-MM-DD HH:mm:ss') : null,
      allowRepeat: settings.allowRepeat,
      timeLimit: settings.timeLimit,
      status: 'PUBLISHED',
      questions: [
    ...
    if (surveyId) {
      // 如果有问卷ID，更新问卷
      surveyData.id = surveyId;
      survey = await surveyApi.update(surveyData);
    } else {
      // 如果没有问卷ID，创建新问卷
      survey = await surveyApi.create(surveyData);
      // 保存问卷ID到路由参数
      route.params.id = survey.id;
    }
    // 调用发布接口
    await surveyApi.publish(survey.id);
    message.success('🚀 问卷已发布！已生效答题开始/截止时间、重复答题限制及限时要求。\n可分享链接/二维码给答题者。');
  } catch (error) {
  ...
  try {
    ...
    if (surveyId) {
      ...
      const surveyData = {
        ...
        allowRepeat: settingData.allowRepeat,
        timeLimit: settingData.timeLimit
      };
      await surveyApi.update(surveyData);
      message.success('✅ 设置已保存');
    } else {
      message.error('请先保存问卷');
    }
  } catch (error) {
...
const deleteQuestion = (id) => {
  if (confirm('确定要删除该题目吗？此操作不可恢复。')) {
    const currentQuestions = topTab.value === 'basic' ? basicQuestions : questionQuestions;
...
const moveQuestion = (id, direction) => {
  ...
  if (index !== -1) {
    ...
    } else if (direction === 'down' && index < currentQuestions.length - 1) {
      // 下移
      [currentQuestions[index], currentQuestions[index + 1]] = [currentQuestions[index + 1], currentQuestions[index]];
    }
  }
...
const handleQuestionTypeChange = (question) => {
  if (question.type === 'multiple') {
    ...
    delete question._correctAnswer;
    // 为多选题添加属性监听
    Object.defineProperty(question, 'correctOptions', {
      get() {
        return this._correctOptions || [];
...
const saveNewQuestion = () => {
  ...
  if (topTab.value !== 'basic') {
    ...
    } else if (questionToAdd.type === 'input') {
      Object.defineProperty(questionToAdd, 'correctAnswer', {
        get() {
          return this._correctAnswer || '';
        },
        set(value) {
...
<template>
  <div class="app-container">
    <div class="main-content">
      <div class="toolbar">
        <div class="btn-group">
          <a-button @click="openPreview">👁️ 题目预览</a-button>
      ...
      <div class="survey-title-card">
        ...
        <a-textarea v-model:value="surveyDescription" placeholder="请输入问卷描述" style="margin-top: 12px; width: 100%;" />
      </div>
        ...
        <a-tabs v-model:activeKey="topTab">
          <a-tab-pane key="basic" tab="基本信息">
            <div class="editor-area">
              ...
              <!-- 题目列表 -->
              <div v-for="(question, questionindex) in basicQuestions" :key="question.id" class="question-card">
                <div class="question-header">
                  <div style="display: flex; align-items: center; gap: 20px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                      <span style="font-size: 12px; font-weight: 600;">是否必填</span>
                      <a-switch v-model:checked="question.required" size="small" />
                    </div>
                  </div>
                  <div class="question-actions">
                    <a-button size="small" @click="moveQuestion(question.id, 'up')" :disabled="(topTab === 'basic' ? basicQuestions : questionQuestions).indexOf(question) === 0">⬆️ 上移</a-button>
                    <a-button size="small" @click="moveQuestion(question.id, 'down')" :disabled="(topTab === 'basic' ? basicQuestions : questionQuestions).indexOf(question) === (topTab === 'basic' ? basicQuestions : questionQuestions).length - 1">⬇️ 下移</a-button>
                ...
                <div class="question-text">
                  <span class="required-badge" v-if="question.required">*</span>
                  {{ questionindex + 1 }}.&nbsp;<a-input v-model:value="question.text" placeholder="请输入题目内容" style="width: 100%;" />
                </div>
          ...
          <a-tab-pane key="questions" tab="问卷题目">
            <div class="editor-area">

              <!-- 题目列表 -->
              <div v-for="(question, questionindex) in questionQuestions" :key="question.id" class="question-card">
                <div class="question-header">
                  <div style="display: flex; align-items: center; gap: 20px;">
                    <div class="question-type">
                      <a-radio-group v-model:value="question.type" size="small" @change="handleQuestionTypeChange(question)">
                ...
                <div class="options-area">
                  ...
                  <template v-else-if="question.type === 'input'">
                    <div style="margin-bottom: 12px; display: flex; align-items: center; gap: 12px;">
                      ...
                      <a-input v-model:value="question.correctAnswer" placeholder="请输入正确答案" style="flex: 1;" />
                    </div>
                  </template>
                  <template v-else>
    ...
    <a-modal
      ...
      :title="`📱 问卷预览 (答题者视角) - ${previewStep === 'basic' ? '基本信息' : '问卷题目'}`"
      width="700px"
    >
      <h4>{{ surveyTitle }}</h4>
      <div style="margin-top: 20px;">
        <!-- 基本信息题目 -->
        <div v-if="previewStep === 'basic' && basicQuestions.length > 0">
          <div v-for="(question, index) in basicQuestions" :key="question.id" style="margin-bottom: 20px;">
              <p><strong>{{ index + 1 }}. {{ question.text }}<span v-if="question.required" style="color: #ef4444; font-weight: normal; margin-left: 8px;">* 必填</span></strong></p>
            <div style="margin-top: 10px; margin-left: 20px;">
              ...
              <template v-else-if="question.type === 'input'">
                <a-input v-model:value="previewAnswers[question.id]" placeholder="请输入答案" style="width: 100%;" />
              </template>
              <template v-else>
        ...
        <!-- 问卷题目 -->
        <div v-else>
          <div v-for="(question, index) in questionQuestions" :key="question.id" style="margin-bottom: 20px;">
              <p><strong>{{ index + 1 }}. {{ question.text }}<span v-if="question.required" style="color: #ef4444; font-weight: normal; margin-left: 8px;">* 必填</span></strong></p>
            <div style="margin-top: 10px; margin-left: 20px;">
              <template v-if="question.type === 'single'">
                <a-radio-group v-model:value="previewAnswers[question.id]" style="display:inline-grid">
      ...
      <template #footer>
        <div style="display: flex; justify-content: space-between; width: 100%;">
          <div>
            <a-button v-if="previewStep === 'questions'" @click="previewStep = 'basic'">上一步</a-button>
          </div>
          <div>
            <a-button v-if="previewStep === 'basic' && basicQuestions.length > 0" @click="previewStep = 'questions'">下一步</a-button>
    ...
    >
      ...
      <!-- 必填设置 -->
      <div style="margin-bottom: 20px;">
        <label style="display: block; font-weight: 600; margin-bottom: 8px;">是否必填</label>
        <a-switch v-model:checked="newQuestion.required" />
        <span style="margin-left: 8px; font-size: 12px;">{{ newQuestion.required ? '必填' : '非必填' }}</span>
      </div>
      ...
      <!-- 选项编辑 -->
      <div style="margin-bottom: 20px;">
        <template v-if="newQuestion.type === 'input'">
        </template>
        <template v-else>
...
.top-tabs .ant-tabs-tab-active {
  color: #2563eb;
}

.top-tabs .ant-tabs-ink-bar {
  background: #2563eb;
  height: 3px;
  border-radius: 1.5px;
}
